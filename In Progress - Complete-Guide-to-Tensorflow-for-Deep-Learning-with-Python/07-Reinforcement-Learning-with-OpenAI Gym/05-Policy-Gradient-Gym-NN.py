...
# Creating placeholder for the inputs
X = tf.placeholder(tf.float32, 
                shape = [None, num_inputs])

# Hidden layer with 4 hidden units
hidden_layer = tf.layers.dense(X, num_hidden, 
                            activation = tf.nn.elu, 
                            kernel_initializer = initializer)
# Output layer before sigmoid activation function
logits = tf.layers.dense(hidden_layer, num_outputs)
outputs = tf.nn.sigmoid(logits)  # probability of action 0 (left)

probabilties = tf.concat(axis = 1, 
                        values = [outputs, 1 - outputs])
# Sampling action from the probability
...
# Compute gradients instead of minimizing the loss
...
def helper_discount_rewards(rewards, discount_rate):
    ...
    '''
    # Initialize discounted rewards as numpy array of zeros
